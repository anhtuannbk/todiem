...
def load_excel_data(excel_path):
    try:
        ...
        absent_count = sum(1 for score in grades.values() if pd.isna(score))
        print(f"Số lượng sinh viên trong file Excel: {len(grades)}")
        print(f"Số lượng sinh viên vắng: {absent_count}")
        return grades, absent_count
...
def add_grade_to_pdf(input_pdf, output_pdf, grades, absent_count, total_students, info):
    ...
    for page_num in range(total_pages):
        ...
        if page_num == 0:
            ...
            can.drawCentredString(505, 53, f"{info['grader1']}")
...
def process_files(folder, keyword, info):
    ...
    for pdf_file in pdf_files:
        ...
        grades, absent_count = load_excel_data(excel_file)
        if grades is None:
            continue
        student_positions = extract_student_positions(pdf_path)
        total_students = len(student_positions)
        add_grade_to_pdf(pdf_path, output_pdf, grades, absent_count, total_students, info)
